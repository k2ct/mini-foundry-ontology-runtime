...
PO-002 是一个高金额采购订单（¥150,000），初始状态为 `pending_review`。其关联供应商为"云擎数据中心解决方案"（risk_level: medium），触发高金额风险信号（risk_002）。Agent 分析后建议 `escalate_order`，Action Runtime 执行后订单状态从 `pending_review` 变为 `escalated`，Timeline 可以追溯风险信号、政策依据、AgentRun 和 ActionAuditLog。
